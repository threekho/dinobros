...
function initPokiSdk() { if (!pokiDebug) {} PokiSDK.init().then(() => { setPokiInited(); }).catch(() => { setPokiInited(); }); }
...
function pokiSendLevelData(){navigator.sendBeacon('https://leveldata.poki.io/fancade-5FFEEA620BE54704','9bca26a4-03eb-44bc-9d49-e28e21da8eaf');}
...
function adRewardedShow(){simpleLogC("adRewardedShow()");pokiEnsureStop();PokiSDK.rewardedBreak().then((success)=>{Module.ccall("ad_rewarded_on_showed","v",["number"],[success?1:0]);if(success){Module.ccall("ad_rewarded_on_rewarded","v");}});}
